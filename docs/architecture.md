...
Qwen Cloud / DashScope 属于外部模型服务，只通过后端 LLM Client 访问；前端和数据库不直接连接 Qwen。
...
flowchart TB
    ...
    subgraph L2["2. 接入与运行时层"]
        FastAPI["FastAPI 后端<br/>/api/* + /dashboard"]
        Runtime["Agent Runtime<br/>Gateway / Scheduler / Brain"]
        SDK["Agent Eval SDK / OTel Adapter"]
    end

    subgraph L3["3. 应用服务层"]
        CaseService["用例与测试集管理<br/>Case / CaseSet"]
        TaskService["评测任务管理<br/>EvalTask / EvalRun"]
        IngestService["Trace 采集服务<br/>IngestWorker"]
        EvalService["评测执行服务<br/>evaluate_trace()"]
        BrainProxy["Brain Console 代理"]
    end

    subgraph L4["4. 领域能力层"]
        Orchestrator["EvaluationOrchestrator<br/>五层评测编排"]
        Evaluators["Evaluators<br/>Intent / Retrieval / Tool / Generation / Outcome"]
        LLMClient["LLM Client<br/>LLMJudge / LLMIntentParser"]
    end

    subgraph L5["5. 基础设施层"]
        Redis["Redis<br/>事件缓冲 / 队列"]
        PostgreSQL["PostgreSQL<br/>Trace / Case / Run / Score"]
