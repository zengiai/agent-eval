...
| `spans` | Trace 中的阶段（意图/召回/工具/生成） | 1:1 → eval_scores |
| `eval_scores` | 评测器对 Span 的打分明细 | — |
...
| 实体 | score 字段 | 含义 | 计算方式 |
|------|-----------|------|---------|
| spans | `spans.score` | 该阶段的得分 | 由 eval_scores 计算后回填 |
| traces | `traces.overall_score` | 整条链路的综合得分 | Span 得分加权平均 |
| eval_scores | `eval_scores.score` | 评测器的打分输出 | 评测器计算 |
...
对比前置条件：两者使用相同的 `case_set_id` 和 `evaluator_version`，确保公平。
...
-- 从生产 Trace 中创建 Case
INSERT INTO eval_cases (case_set_id, query, source, source_trace_id)
SELECT 'set_xxx', query, 'trace', id FROM traces WHERE id = 'trace_yyy';
```
