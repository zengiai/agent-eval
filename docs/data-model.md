...
| `spans` | Trace 中的阶段（intent/retrieval/tool_call/generation） | 1:1 → eval_scores（仅前四层） |
| `eval_scores` | 评测器对 Span 的打分明细；Outcome 层 span_id=NULL；trace_id 关联 Trace | 可空 FK → spans；NOT NULL FK → traces |
...
| eval_scores | `eval_scores.score` | 评测器的打分输出 | 评测器计算；Outcome 层 `span_id = NULL`，通过 `trace_id` 关联 Trace |
